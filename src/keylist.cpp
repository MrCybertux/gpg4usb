...
KeyList::KeyList(GpgME::GpgContext *ctx, QWidget *parent) : QWidget(parent)
{
    mCtx = ctx;

    mKeyList = new QTableWidget(this);
    // Now 7 columns: check, icon, name, type, email, id, fpr
    ...
    // hide id and fingerprint of key (columns 5 and 6)
    ...
    QStringList labels;
    labels << "" << "" << tr("Name") << tr("Type") << tr("EMail") << "id" << "fpr";
...
void KeyList::slotRefresh()
{
    ...
    while (it != keys.end()) {
        ...
        // Type column (algorithm + optional curve)
        QString typeStr;
        if (!it->algo.isEmpty()) {
            typeStr = it->algo;
            if (!it->curve.isEmpty())
                typeStr += QString(" / %1").arg(it->curve);
        }
        QTableWidgetItem *tmpType = new QTableWidgetItem(typeStr);
        tmpType->setToolTip(typeStr);
        mKeyList->setItem(row, 3, tmpType);
        ...
        if(it->expired || it->revoked) {
            ...
            tmp2->setFont(strike);
            tmp3->setFont(strike);
            tmpType->setFont(strike);
        }

        // email now column 4
        mKeyList->setItem(row, 4, tmp3);
        QTableWidgetItem *tmp4 = new QTableWidgetItem(it->id);
        mKeyList->setItem(row, 5, tmp4);
        QTableWidgetItem *tmp5 = new QTableWidgetItem(it->fpr);
        mKeyList->setItem(row, 6, tmp5);
...
void KeyList::setChecked(QStringList *keyIds)
{
    if (!keyIds->isEmpty()) {
        for (int i = 0; i < mKeyList->rowCount(); i++) {
            if (keyIds->contains(mKeyList->item(i, 5)->text())) {
